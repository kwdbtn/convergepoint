    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">{{ $virtualMeter->name }} <span style="color: red">||</span> {{ $variableKey }}
                <span class="float-right"><a href="{{ route('customer-readings.show', [$reading, $customer]) }}" class="btn btn-sm btn-dark float-end">Back</a></span>
            </h5>
            <hr>
                    ...
                    <div class="card mb-2" style="height: 250px; overflow:auto;">
                        ...
                        <div class="card-body">
                            @foreach ($variables as $key => $variable)
                                <li><a style="{{ $key == $variableKey ? 'color: black; font-weight: bold' : 'text-decoration: none'}}" href="{{ route('customer-readings.show-meter-readings', [$reading, $customer, $virtualMeter, $variable]) }}">{{ $key }}</a></li> <hr>
                            @endforeach
                        </div>
                    </div>
